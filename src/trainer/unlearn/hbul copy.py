...
class HBUL(UnlearnTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        def get_h_trigger(last_hidden_states, labels):
            """
            ...
            def first_content_index(labels, input_ids, filler_ids):
                ...
                for b in range(B):
                    ...
                    while i < T and mask[b, i] and int(model_inputs["input_ids"][b, i]) in filler_ids:
                        i += 1
                    first_content[b] = i if i < T else first_idx[b]
        ...
        forget_embeddings_euclidean, pre_idx, has_ans = get_h_trigger(last_hidden_states, labels)
        if self.clip_embeddings:
