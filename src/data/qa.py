...
class QADatasetHyp(Dataset):
    def __init__(
        ...
        super(QADatasetHyp, self).__init__()
        self.tokenizer = tokenizer
        self.max_length = max_length
        self.data = load_hf_dataset(**hf_args)
        self.data = add_dataset_index(self.data)
    ...
    def _process_sample(self, question, answer, index=-1):
        ...
        if self.fs_data is None:
            ...
            if answer.startswith(" "):
                response_msgs = [ans_token + answer]
            else:
                response_msgs = [ans_token + " " + answer]
        else:
            prompt_msgs = self.fs_data[self.question_key] + [question]
            ...
            fs_answers = []
            for ans in self.fs_data[self.answer_key]:
                if ans.startswith(" "):
                    fs_answers.append(ans_token + ans)
                else:
                    fs_answers.append(ans_token + " " + ans)
            # Current answer
            if answer.startswith(" "):
                curr_ans = ans_token + answer
            else:
                curr_ans = ans_token + " " + answer
            response_msgs = fs_answers + [curr_ans]
        ans_token_id = self.tokenizer.convert_tokens_to_ids(ans_token)
        # print(f">>>>>>>>>>{prompt_msgs}-{response_msgs} | {ans_token} token id: {ans_token_id}")
        tokenized_data = preprocess_chat_instance(
            self.tokenizer,
            self.template_args,
            prompt_msgs,
